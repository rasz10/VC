<?php
	...
	for($i=0; $i<$NANO; $i++){
		if ($status[$i] != "-") {
			...
			$monitoring = "UPDATE acc_monitoring SET validasipencairan_kakaper='$status[$i]',user_validasipencairan_kakaper='$user[$i]"."|"."$date[$i]',note_validasipencairan_kakaper='$catatan[$i]' where iddebitur='$iddebitur[$i]'";
			// echo $monitoring."<br>";
