<?php
	...
	for($i=0; $i<$NANO; $i++){
		if ($status[$i] != "-") {
			...
			$monitoring = "UPDATE acc_monitoring SET verifikasipencairan_kdo_legal='$status[$i]',user_verifikasipencairan_kdo_legal='$user[$i]"."|"."$date[$i]',note_verifikasipencairan_kdo_legal='$catatan[$i]' where iddebitur='$iddebitur[$i]'";
			// echo $monitoring."<br>";
